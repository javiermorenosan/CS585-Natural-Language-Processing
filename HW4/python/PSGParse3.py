...
##  rule_re = re.compile(r"(?P<lhssymbol>[^-=>#,\s]+([-]+[^-=>#,\s]+)*)\s+(-+|=+)>\s+(?P<rhssymbols>[^-=>#,\s]+([-]+[^-=>#,\s]+)*(\s+[^-=>#,\s]+([-]+[^-=>#,\s]+)*)*)(\s*#.*)?")
  ...
  def __init__(self, filename):
    ...
    self.lhshash      = {}
    self.rhshash      = {}
    # New vairable. List of probabilitites of each rule
    self.probs        = []
    # New variable. Dictionary with rules as keys and probabilities as values.
    self.rule_probs   = {}
    self.load(filename)
  ...
  def load(self, filename):
    ...
    self.rule_probs = dict(zip(rules.keys(), self.probs))
